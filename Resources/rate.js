	...
	*/
	...
	var addDays = function(myDate,days) {
		return  myDate + days*24*60*60;
	}
	
	/**
	* Check Reminder To Rate function will show a window that has an alert to rate this app.
	*/
	var checkReminderToRate = function() {
		...
		if(!remindToRate) {
			...
			var days = remindToRate;
		}
		Ti.API.info(days);
		
		if (!remindToRate) {
			/**
			* Set number of days to wait until you want to remind the user to rate your app
			*/
			var days = addDays(now,4);
			Ti.App.Properties.setString('RemindToRate', days);
		}
		else if (remindToRate < now) {
			var alertDialog = Titanium.UI.createAlertDialog({
				title: 'Please rate this app!',
				message: 'Would you take a moment to rate this app?',
				buttonNames: ['OK', 'Later', 'Never'],
				cancel: 2
			});
			alertDialog.addEventListener('click', function(evt) {
				switch (evt.index) {
					case 0:
						...
						if (Ti.Android) {
							Ti.Platform.openURL('https://market.android.com/details?id=lu.artou.marketenablercodes');
						}else {
					...
					case 1:
						...
						Ti.App.Properties.setString('RemindToRate',  addDays(now,4));
						break;
					case 2:
						Ti.App.Properties.setString('RemindToRate', Number.MAX_VALUE);
						break;
